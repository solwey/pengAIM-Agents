...
from aegra_api.services.worker_queue import encode_queue_payload
from aegra_api.settings import settings

logger = structlog.getLogger(__name__)
RunRef = tuple[str, str]  # (tenant_schema, run_id)
...
class LeaseReaper:
    ...
    @staticmethod
    async def _find_recoverable() -> tuple[list[RunRef], list[RunRef]]:
        ...
        async with maker() as session:
            crashed_result = await session.execute(select(Tenant).where(Tenant.enabled.is_(True)))
            tenants = crashed_result.scalars().all()

        crashed: list[RunRef] = []
        stuck_pending: list[RunRef] = []
        for tenant in tenants:
            async with new_tenant_session(tenant.schema) as tenant_session:
                crashed_result = await tenant_session.execute(
                ...
                crashed.extend((tenant.schema, row[0]) for row in crashed_result.fetchall())

                stuck_result = await tenant_session.execute(
                    select(RunORM.run_id).where(
                ...
                stuck_pending.extend((tenant.schema, row[0]) for row in stuck_result.fetchall())

        return crashed, stuck_pending

    @staticmethod
    async def _reset_to_pending(run_refs: list[RunRef]) -> list[RunRef]:
        """Reset crashed runs to pending. Re-checks lease expiry atomically."""
        reset_refs: list[RunRef] = []
        by_schema: dict[str, list[str]] = {}
        for schema, run_id in run_refs:
            by_schema.setdefault(schema, []).append(run_id)

        for schema, run_ids in by_schema.items():
            async with new_tenant_session(schema) as session:
                ...
                reset_refs.extend((schema, run_id) for run_id in reset_ids)
        return reset_refs

    @staticmethod
    async def _reenqueue(run_refs: list[RunRef]) -> None:
        queue_key = settings.worker.WORKER_QUEUE_KEY
        try:
            client = redis_manager.get_client()
            for schema, run_id in run_refs:
                payload = encode_queue_payload(schema, run_id)
                await client.rpush(queue_key, payload)  # type: ignore[arg-type]
                logger.info("Re-enqueued recovered run", run_id=run_id, tenant_schema=schema)
        except RedisError:
            logger.warning(
                "Redis unavailable during re-enqueue; workers will pick up via Postgres poll",
                run_ids=[run_id for _, run_id in run_refs],
            )

    @staticmethod
    async def _check_retry_limits(run_refs: list[RunRef]) -> tuple[list[RunRef], list[RunRef]]:
        ...
        retryable: list[RunRef] = []
        exhausted: list[RunRef] = []
        by_schema: dict[str, list[str]] = {}
        for schema, run_id in run_refs:
            by_schema.setdefault(schema, []).append(run_id)
            ...
            async with new_tenant_session(schema) as session:
                for run_id in run_ids:
                    ...
                    if retry_count > max_retries:
                        exhausted.append((schema, run_id))
                        logger.error(
                        ...
                        await session.execute(
                            update(RunORM).where(RunORM.run_id == run_id).values(execution_params=params)
                        )
                        retryable.append((schema, run_id))
                        logger.info(
    ...
    @staticmethod
    async def _mark_permanently_failed(run_refs: list[RunRef]) -> None:
